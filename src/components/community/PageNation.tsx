	...
	return (
		<SC.FlexBox $gap={30} style={{ margin: "0 auto" }}>
			<SC.RankNum
				...
				$bColor='lightgray3'
				onClick={onNavigate({ url: `/community/${getId && getId - 1}` })}
				children={<p children={`<`} />}
			/>
			<SC.FlexBox $gap={10}>
				<SC.RankNum
					$state={pageNum === 1}
					$bColor='lightgray2'
					onClick={onNavigate({ url: `/community/${pageNum - 4}` })}
					children={<p children='...' />}
				/>
				{data && pageNum + 4 >= data.totalPages
					? Array.from({ length: data.totalPages - pageNum + 1 }, (_, idx) => idx).map((list) => (
							<SC.RankNum
								key={list}
					...
					: Array.from({ length: 5 }, (_, idx) => idx).map((list) => (
							<SC.RankNum
								key={list}
								$bColor={getId === pageNum + list ? `blue` : "lightgray2"}
								onClick={onNavigate({ url: `/community/${pageNum + list}` })}
