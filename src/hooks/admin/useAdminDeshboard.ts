...
export const useAdminDeshboard = ():any => {
    ...
    const onPeriodSelect = (e: MouseEvent<HTMLDivElement>) => {
      ...
      switch (valueArr && valueArr[1]) {
        ...
        case "month":
          ...
          setShowPeriod(`${selectDate.startOf('month').format("YY/MM/DD")} ~ ${selectDate.endOf('month').format("MM/DD")}`)
          return
        default:
    ...
    const onBeforePeriod = () => {
      switch (getType[1]) {
        case "week":
          ...
          setShowPeriod(`${minusWeek.startOf('week').format("MM/DD")} ~ ${minusWeek.endOf('week').format("MM/DD")}`)
          return
        case "month":
          const minusMonth = dayjs(period).subtract(1, 'month')
          setAfterPeriodState(true)
          setPeriod(minusMonth.format("YYYY-MM"))
          setShowPeriod(`${minusMonth.startOf('month').format("YY/MM/DD")} ~ ${minusMonth.endOf('month').format("MM/DD")}`)
          return
        default:
          const minusYear = dayjs(period).subtract(1, 'year')
          setAfterPeriodState(true)
          setPeriod(minusYear.format("YYYY"))
          setShowPeriod(`${minusYear.startOf('year').format("YY.MM")} ~ ${minusYear.endOf('year').format("YY.MM")}`)
          return
      }
    }
    ...
    const onAfterPeriod = () => {
      switch (getType[1]) {
        case "week":
          ...
          const afterPeriodStateW = dayjs(period).add(2, 'week').startOf('week').format('YYYY-MM-DD') < selectDate.startOf('week').format("YYYY-MM-DD")
        ...
        case "month":
          ...
          const afterPeriodStateM = dayjs(period).add(2, 'month').startOf('month').format('YYYY-MM-DD') < selectDate.startOf('month').format("YYYY-MM-DD")
        ...
        default:
          ...
          const afterPeriodStateY = dayjs(period).add(2, 'year').startOf('year').format('YYYY-MM-DD') < selectDate.startOf('year').format("YYYY-MM-DD")
    ...
    const { isLoading: userLoading, data: userData, isError: userIsError, error: userError } = useGetUserChartQuery(new Date().getFullYear())
    const userGenderRatio = {
      labels: ["남성", "여성", "알수없음"],
      datasets: [
        {
          data: userData && [userData.gender.ratio.MALE, userData.gender.ratio.FEMALE, userData.gender.ratio.UNKNOWN],
          borderColor: [
            "rgba(76,76,255,0.3)",
            "rgba(252,85,85,0.3)",
    ...
    const { isLoading: purchaseLoading, data: purchaseData, isError: purchaseIsError, error: purchaseError } = useGetPurchaseChartQuery({ type: getType[1], period })
    const purchaseReduce = purchaseData && (purchaseData.purchase.values.reduce((pre: number, cal: number) => pre + cal, 0) || 0);
    const approveReduce = purchaseData && (purchaseData.approve.values.reduce((pre: number, cal: number) => pre + cal, 0) || 0);
    const cancelReduce = purchaseData && (purchaseData.cancel.values.reduce((pre: number, cal: number) => pre + cal, 0) || 0);
    ...
    return {
      ...
      purchaseLoading, purchaseIsError, purchaseError, purchaseData,
