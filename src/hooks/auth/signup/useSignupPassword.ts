...
export const useSignupPassword = ({ name, submitted }: any): any => {
  ...
  const getValidateMsg = RTK.useAppSelector(
    name === "password" ? RTK.selectValiditePMsg : RTK.selectValiditePWCMsg
  );
  ...
  const onValiditeMsgPWC = (input: string): void => {
    input === ""
      ...
      ? dispatch(
          RTK.setValiditeMsg({
            type: name,
            msg: ["입력하신 비밀번호가 서로 다릅니다.", false],
          })
        )
      : dispatch(
          RTK.setValiditeMsg({
            type: name,
            msg: ["비밀번호가 일치합니다. ", true],
          })
        );
  };
  ...
  const onChangeInput = (e: ChangeEvent<HTMLInputElement>) => {
    name === "password"
      ? onValiditeMsgP(e.target.value)
      : onValiditeMsgPWC(e.target.value);
    setInput(e.target.value);
  };

  // 패스워드만 onBlur했을 때, 디스패치로 데이터에 추가
  const onBlurSignupDispatch = () => {
    name === "password" && dispatch(RTK.setSignupDate({ [`${name}`]: input }));
  };
