...
export const useCommunityWrite = () => {
	...
	const onSubmitReview = async (e: FormEvent<HTMLFormElement> | MouseEvent<HTMLDivElement>) => {
		e.preventDefault();
		...
		try {
			const result = await onPostCommunity(formData).unwrap();
			if (result && result.data && result.data.postId) {
				const newPostId = result.data.postId;
				onNavigate({ url: `/community/review/${newPostId}` })();
			} else {
				console.log("Post ID is missing in the result");
			}
		} catch (error) {
