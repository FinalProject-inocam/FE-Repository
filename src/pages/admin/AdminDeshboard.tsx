...
import { Chart as ChartJS, BarElement, LineElement, PointElement, LinearScale, CategoryScale, RadialLinearScale, ArcElement, Tooltip, Legend } from "chart.js";
...
dayjs.locale('ko')
ChartJS.register(ArcElement, Tooltip, BarElement, LineElement, PointElement, LinearScale, CategoryScale, RadialLinearScale, Legend);
...
export const AdminDeshboard: React.FC = () => {
  ...
  const {
    ...
    // RTKQ
    userLoading, userIsError, userError, userData,
  ...
  return (
    <SC.DeshboardGrid $gtc="1fr 380px" $gtr="1fr" style={{width:"100%"}}>
      <SC.ChartLayout $bColor="lightgray1" $padding="40px">
        <SC.FlexBox $jc="space-between">
          ...
          <SC.FlexBox $gap={10}>
            <SC.DataTypeSelect $jc="space-around" onClick={onChangeDataTypeSelect}>
              <div>{getType[0]}</div>
              <SC.FigureObjectFitImg width="14px" height="7px" src={ASS.underArrow} alt="underArrow"></SC.FigureObjectFitImg>
              {dataTypeSelect && (
                <SC.DataTypeOptions>
            ...
            <SC.DateShowPeriod $jc="space-between">
              {/* 날짜 이동 버튼  */}
              <SC.FigureObjectFitImg types="cursor" onClick={onBeforePeriod} width="16px" height="16px" src={ASS.leftArrowB} alt="leftArrow" />
              <div>{showPeriod}</div>
              <SC.FigureObjectFitImg types={afterPeriodState ? "cursor" : "none"} onClick={onAfterPeriod} width="16px" height="16px" src={afterPeriodState ? ASS.rigthArrowB : ASS.rigthArrowG} alt="rigthArrow" />
            </SC.DateShowPeriod>
        ...
        <SC.GridBox $gtc="1fr" $gtr="180px repeat(2, 1fr)" $rgap={20} $jc="flex-start" style={{ height: "calc(100% - 40px)", marginTop: "10px" }}>
          <SC.GridBox $gap={10}>
              ...
              <SC.GridBox $gtc="repeat(3, 1fr)" style={{ height: "100%" }} $gap={4}>
                {userLoading
                  ...
                  : userIsError
                    ...
                    : (<>
                      <SC.FlexBox $gap={3} $fd="column" $ai="flex-start" style={{ padding: "0 25px", border: "1px solid #DEDEE0", borderRadius: "10px" }}>
                        <SC.CustomH3 $size={0.75} $color="textColorSub" children="출고현황" />
                        <SC.CustomH3 $size={1.5} children={<>13<span children="건" /></>} />
                        <SC.CustomH3 $size={0.75} $color="textColorSub" children={`전주, 7명`} />
                      </SC.FlexBox>
                      <SC.FlexBox $gap={3} $fd="column" $ai="flex-start" style={{ padding: "0 25px", border: "1px solid #DEDEE0", borderRadius: "10px" }}>
                        <SC.CustomH3 $size={0.75} $color="textColorSub" children="출고승인" />
                        <SC.CustomH3 $size={1.5} children={<>5<span children="건" /></>} />
            ...
            <SC.GridBox $gtc="300px 2fr" style={{ width: "100%", height: "100%" }} $cgap={10}>
              <SC.PurchaseSecondRowInner $fd="column" $jc="flex-start" $ai="flex-start" $gap={10}>
                ...
                <SC.FlexBox style={{ height: "90%", margin: "0 auto" }} $gap={10}>
                  <div style={{ width: "120px", height: "120px" }}><Doughnut options={dataRatioOpts} data={purchaseRatio} /></div>
                      ...
                      : userIsError
                        ...
                        : (<>
                          ...
                          <SC.FlexBox $gap={5}>
                            ...
                            <SC.CustomH3 $size={0.875} children={`${((approveReduce / purchaseReduce) * 100).toFixed(1)}%`} />
                          </SC.FlexBox>
                          <SC.FlexBox $gap={5}>
                            ...
                            <SC.CustomH3 $size={0.875} children={`${((cancelReduce / purchaseReduce) * 100).toFixed(1)}%`} />
                          </SC.FlexBox>
      ...
      <SC.ChartLayout $bColor="blue" $padding="40px 10px">
        ...
        <SC.GridBox $gtc="1fr" $gtr="180px repeat(2, 1fr)" style={{ height: "calc(100% - 40px)", marginTop: "10px" }} $rgap={10}>
                ...
                : userIsError
                  ...
                  : (<>
                    <SC.FlexBox $gap={3} $fd="column" $ai="flex-start" style={{ padding: "0 25px", border: "1px solid #DEDEE0", borderRadius: "10px" }}>
                      ...
                      <SC.CustomH3 $size={1.5} children={<>13<span children="건" /><span style={{ color: 13 - userData.preUser >= 0 ? "red" : "blue", marginLeft: "5px", fontSize: "12px" }} children={`${13 - userData.preUser >= 0 ? "↑" + (13 - userData.preUser) : "↓" + (13 - userData.preUser)}명`} /></>} />
                      <SC.CustomH3 $size={0.75} $color="textColorSub" children={`작년, ${userData.preUser}명`} />
                    </SC.FlexBox>
                    <SC.FlexBox $gap={3} $fd="column" $ai="flex-start" style={{ padding: "0 25px", border: "1px solid #DEDEE0", borderRadius: "10px" }}>
                      <SC.CustomH3 $size={0.75} $color="textColorSub" children="이번달 가입 회원 수" />
                      <SC.CustomH3 $size={1.5} children={<>5<span children="건" /></>} />
                      <SC.CustomH3 $size={0.75} style={{ visibility: "visible", height: "15px" }} />
                    </SC.FlexBox>
                  </>)
          ...
          <SC.ChartInner>
            <SC.FlexBox $fd="column" $jc="flex-start" $ai="flex-start" $gap={10} style={{ width: "100%", height: "100%" }}>
              <SC.CustomH1 $size={1.125} children="회원 성별통계" />
              <SC.FlexBox style={{ height: "90%", margin: "0 auto" }} $gap={10}>
                <div style={{ width: "150px", height: "150px" }}><Doughnut options={dataRatioOpts} data={userGenderRatio} /></div>
                <SC.FlexBox $fd="column" $gap={17} $ai="flex-start">
                  {userLoading
                    ? <div>로딩 중... </div>
                    : userIsError
                      ...
                      : (<>
                        <SC.FlexBox $gap={5}>
                          ...
                          <SC.CustomH3 $size={0.875} children={`${userData.gender.byGender.MALE}명`} />
                          <SC.CustomH3 $size={0.875} children={`${userData.gender.ratio.MALE}%`} />
                        </SC.FlexBox>
                        <SC.FlexBox $gap={5}>
                          <SC.RatioBox $size={14} $bColor="red3" />
                          <SC.CustomH3 $size={0.875} $color="red3" children="여성" />
                          <SC.CustomH3 $size={0.875} children={`${userData.gender.byGender.FEMALE}명`} />
                          <SC.CustomH3 $size={0.875} children={`${userData.gender.ratio.FEMALE}%`} />
                        </SC.FlexBox>
                        <SC.FlexBox $gap={5}>
                          <SC.RatioBox $size={14} $bColor="textColorSub" />
                          <SC.CustomH3 $size={0.875} $color="textColorSub" children="알수없음" />
                          <SC.CustomH3 $size={0.875} children={`${userData.gender.byGender.UNKNOWN}명`} />
                          <SC.CustomH3 $size={0.875} children={`${userData.gender.ratio.UNKNOWN}%`} />
                        </SC.FlexBox>
                      </>)}
                </SC.FlexBox>
              </SC.FlexBox>
          ...
          <SC.ChartInner>
            <SC.FlexBox $fd="column" $jc="flex-start" $ai="flex-start" $gap={10} style={{ width: "100%", height: "100%" }}>
              ...
              <SC.FlexBox style={{ height: "90%", margin: "0 auto" }} $gap={10}>
                <div style={{ width: "150px", height: "150px" }}><PolarArea options={dataRatioOpts} data={userAgeRatio} /></div>
                    ...
                    : userIsError
                      ...
                      : (<>
                        ...
                        <SC.FlexBox $gap={5}>
                          ...
                          <SC.CustomH3 $size={0.875} children={`${userData.age.byAge['70+']}명`} />
