...
export const Signup: React.FC = () => {
  ...
  const { onSubmitSign, signInfo, onChangeInput, onCheckEmail, onCheckNickName,
    ...
    checkNickNameSuccess, checkNickNameData } = useSignup()

  useEffect(() => {
    if (isSuccess) onNavigate('/login')()
    if (checkEmailSuccess) console.log(checkEmailData ? "사용 가능한 이메일 입니다" : "이미 사용 중인 이메일입니다")
    if (checkNickNameSuccess) console.log(checkNickNameData ? "사용 가능한 닉네임 입니다" : "이미 사용 중인 닉네임입니다")

  }, [isSuccess, data, isError, error, onNavigate,
    checkEmailSuccess, checkEmailData, checkNickNameSuccess, checkNickNameData])
  ...
  return (
    <form onSubmit={onSubmitSign}>
      ...
      <input type='text' value={signInfo.phone_number} name='phoneNumber' onChange={onChangeInput} placeholder='휴대전화를 입력해 주세요' />
